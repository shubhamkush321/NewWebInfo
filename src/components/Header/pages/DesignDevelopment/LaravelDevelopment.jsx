...
const LaravelDevelopment = () => {
  ...
  const LaravelDevelopmentData = infoDetails?.find(
    (item) => item?.items[0].title === "Laravel Web Development Company"
  );

  return (
    <>
      {LaravelDevelopmentData && (
        <div className="lg:p-44 text-gray-800 bg-gray-100 -mt-20 justify-center">
          <h1 className="text-3xl font-bold mb-6 text-gray-800 lg:text-center uppercase">
            {LaravelDevelopmentData?.items[0]?.title}
            &nbsp;
          </h1>
          ...
          <div className="mb-8">
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: LaravelDevelopmentData?.items[0]?.content,
              }}
            />
          </div>
        </div>
