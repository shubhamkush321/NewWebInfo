...
const SeoAudit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${SERVERAPI}/api/audit-content`);
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white mt-20">
      {data.map((section, index) => {
        ...
        switch (type) {
          ...
          case 'paragraph':
            return (
              <p key={index} className="text-lg text-gray-700 mb-6" dangerouslySetInnerHTML={{ __html: text }} />
            );
          case 'list':
            return (
              <ul key={index} className="list-disc list-inside pl-5 mb-6 text-lg text-gray-700">
                {items.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
